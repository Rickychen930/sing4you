...
export const CTASection: React.FC<CTASectionProps> = memo(({
  ...
  return (
    <SectionWrapper
      ...
      className={cn(
        'bg-gradient-to-br from-gold-900/18 via-jazz-900/22 to-musical-900/18',
        'relative overflow-hidden',
        'border-y border-gold-800/30',
        className
      )}
    >
      {/* Subtle gradient glow - CSS only, perf-safe */}
      <div
        className="absolute inset-0 opacity-[0.06] pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse 80% 50% at 50% 50%, rgba(255,194,51,0.3) 0%, transparent 70%)',
        }}
        aria-hidden
      />
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-stretch sm:items-center">
            <Button
              ...
              className="w-full sm:w-auto group shadow-[0_6px_20px_rgba(255,194,51,0.4)] hover:shadow-[0_8px_28px_rgba(255,194,51,0.5)]"
              aria-label="Contact via WhatsApp"
            >
              <span className="flex items-center justify-center gap-2">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 group-hover:translate-x-0.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <Button
              ...
              size="lg"
              onClick={handleEmail}
              className="w-full sm:w-auto border-2 border-white/80 text-white hover:bg-white/15 hover:border-white backdrop-blur-sm group"
