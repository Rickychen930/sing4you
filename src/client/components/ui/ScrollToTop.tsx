...
export const ScrollToTop: React.FC = memo(() => {
  ...
  const handleScroll = useCallback(() => {
    if (!ticking.current) {
      window.requestAnimationFrame(() => {
        // Use window.scrollY instead of pageYOffset for better performance
  ...
  return (
    ...
    >
      <div className="absolute -inset-1 bg-gradient-to-r from-gold-400 via-gold-400 to-gold-400 rounded-full opacity-0 group-hover:opacity-40 transition-opacity duration-300 blur-sm pointer-events-none" aria-hidden />
      <div className="absolute -inset-2 bg-gradient-to-r from-gold-500 via-musical-500 to-gold-500 rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-300 blur-md pointer-events-none" aria-hidden />
      <svg
        className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 relative z-10 transition-all duration-300 group-hover:-translate-y-1 group-hover:scale-105 drop-shadow-[0_0_8px_rgba(255,255,255,0.35)] group-hover:drop-shadow-[0_0_12px_rgba(255,194,51,0.45)]"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
        aria-hidden="true"
